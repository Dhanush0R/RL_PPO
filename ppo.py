...
def make_env(seed):
    def thunk():
        ...
        # Remove RecordEpisodeStatistics wrapper entirely
...
def train_agent(config: dict, max_steps: int, seed: int,
    ...
    # Pre-allocate rollout storage tensors
    ...
    # Calculate how many full rollout updates we need
    num_updates = max_steps // batch_size

    # Initialize this before your tqdm loop
        ...
        for update in range(num_updates):
            for step in range(rollout_steps):
                ...
                for i in range(num_envs):
                    if dones_np[i]:
                        ...
                        rewards_list.append(current_ep_rewards[i])
                        current_ep_rewards[i] = 0.0 # reset for the next episode
                # ---------------------------------
                            
                pbar.update(num_envs)

            # 2. Compute GAE over the collected batch
            with torch.no_grad():
            ...
            for epoch in range(ppo_epochs):
                ...
                for start in range(0, batch_size, minibatch_size):
                    ...
                    # --- Update 6: Mini-Batch Advantage Normalization ---
                    mb_advantages = (mb_advantages - mb_advantages.mean()) / (mb_advantages.std() + 1e-8)

                    # Get current network predictions for the mini-batch
                    probs = actor(mb_obs)
                    dist = Categorical(probs)
